...
module.exports = function genWrapTextNode(nodeInfos, functionArray) {

      // debugger

      let richTextNode = {
          tagName: 'rich-text',
          _convertedTagName: "Text",
          computedStyle: {
              display: "inline"
          },
          logic: {
        ...
        if(1) //Text Span mode
        {
          ...
          for (let i = 0; i < nodeInfos.length; i++) {
            ...
            if (!v.tails) {
              ...
              let cmds = v.tails?.cmds;
              v.cmdsNew = cmds.filter(v=>v);

              cmds.forEach(
                cmd => {
                  if (cmd) {
                    let properyKey = cmd.substr(0, cmd.indexOf("("));

                    if (i === 0) {
                      dumpSameInfo[properyKey] = cmd;
                    } else {
                      if (!dumpSameInfo[properyKey]) {
                        //not dump
                      } else {
                        if (dumpSameInfo[properyKey] === cmd) {

                        } else {
                          delete dumpSameInfo[properyKey] ;
                        }
                      }
          ...
          if (Object.keys(dumpSameInfo).length) {
            for (let cmdKey in dumpSameInfo) {
              let cmd = dumpSameInfo[cmdKey];

              nodeInfos.forEach(
                v => {

                  if (!cmd.startsWith(".fontWeight")
                  && !cmd.startsWith(".fontColor")
                  && !cmd.startsWith(".fontSize")
                  ) {
                    v.str = v.str.replace(cmd, "")
                  }

                  v.strNew = (v.strNew || v.str).replace(cmd, "")
                  v.cmdsNew = v.cmdsNew.filter(v => !v.startsWith(cmdKey))
                }
              )
              // debugger
            }
          ...
          if (uuidString[0] === "\"") {
            retStr += '/*uuid=' + uuidString + "*/\n) {\n";
            richTextNode._uuid = uuidString;
          } else {
            retStr += '/*uuid={' + uuidString.trim() + '}*/\n) {\n';
            richTextNode._uuid = uuidString;
          }
          ...
          Object.keys(dumpSameInfo).forEach(
            cmdKey => {
              retStr += dumpSameInfo[cmdKey]
            }
          )

          if (!Object.keys(dumpSameInfo).find(v=>v.startsWith(".backgroundColor"))) {
            // 
          }
